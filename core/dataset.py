...
class MMDataset(Dataset):
    ...
    def __init_mosi(self):
        ...
        if self.mode == 'train':
            # 创建三个模态的缺失率数组
            missing_rate = [np.zeros((len(data[self.mode][self.train_mode+'_labels']), 1)) for i in range(3)]
            
            # 设置语言模态完全缺失（缺失率为1）
            missing_rate[0] = np.ones((len(data[self.mode][self.train_mode+'_labels']), 1))
    ...
    def __getitem__(self, index):
        if (self.mode == 'train') and (index == 0):
            # 创建三个模态的缺失率数组
            missing_rate = [np.zeros((len(self.data[self.mode][self.train_mode+'_labels']), 1)) for i in range(3)]
            
            # 设置语言模态完全缺失（缺失率为1）
            missing_rate[0] = np.ones((len(self.data[self.mode][self.train_mode+'_labels']), 1))
            # 视觉和音频模态保持完整（缺失率为0，已在初始化时设置）
